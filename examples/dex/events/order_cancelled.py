...
if 'OrderCancelled' in l_events:
    if l_events['OrderCancelled']:
        count = 0
        for e_event_block in l_events['OrderCancelled']:
            for e_event in e_event_block:
                tx_event = DEXOrderCancelled(connection_manager, e_event)
...
if l_historic_data:
    ...
    path_file = '{0}_order_cancelled_{1}_{2}.csv'.format(network, from_block, to_block)
