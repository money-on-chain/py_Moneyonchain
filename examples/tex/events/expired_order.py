...
from moneyonchain.tex import MoCDecentralizedExchange, DEXExpiredOrderProcessed
...
logging.basicConfig(level=logging.INFO,
                    ...
                    datefmt='%Y-%m-%d %H:%M:%S',
                    filename='logs/expired_orders.log',
                    filemode='a')
    ...
    if events:
        for event in events:
            if events_functions in event['event']:
                eve = DEXExpiredOrderProcessed(event)
...
if l_events:
    ...
    path_file = '{0}_expired_order_{1}_{2}.csv'.format(config_network, from_block, to_block)
