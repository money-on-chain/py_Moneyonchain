...
from moneyonchain.events import DEXTickStart
...
if 'TickStart' in l_events:
    if l_events['TickStart']:
        count = 0
        for e_event_block in l_events['TickStart']:
            for e_event in e_event_block:
                tx_event = DEXTickStart(connection_manager, e_event)
...
if l_historic_data:
    ...
    path_file = '{0}_tick_start_{1}_{2}.csv'.format(network, from_block, to_block)
