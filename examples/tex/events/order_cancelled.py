...
from moneyonchain.tex import MoCDecentralizedExchange, DEXOrderCancelled
...
logging.basicConfig(level=logging.INFO,
                    ...
                    datefmt='%Y-%m-%d %H:%M:%S',
                    filename='logs/order_cancelled.log',
                    filemode='a')
...
hours_delta = 0
from_block = 3438823  # from block start
to_block = 3458823  # block end or 0 to last block
block_steps = 1000
...
if l_events:
    columns = DEXOrderCancelled.columns()
    path_file = '{0}_order_cancelled_{1}_{2}.csv'.format(config_network, from_block, to_block)
