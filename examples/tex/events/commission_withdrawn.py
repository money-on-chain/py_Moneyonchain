...
from moneyonchain.events import DEXCommissionWithdrawn
...
if 'CommissionWithdrawn' in l_events:
    if l_events['CommissionWithdrawn']:
        count = 0
        for e_event_block in l_events['CommissionWithdrawn']:
            for e_event in e_event_block:
                tx_event = DEXCommissionWithdrawn(connection_manager, e_event)
...
if l_historic_data:
    ...
    path_file = '{0}_commission_withdrawn_{1}_{2}.csv'.format(network, from_block, to_block)
